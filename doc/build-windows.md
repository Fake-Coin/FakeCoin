...
Below are some notes on how to build FakeCoin Core for Windows.

The options known to work for building FakeCoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the FakeCoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the FakeCoin Core applications.
...
Note that for WSL the FakeCoin Core source path MUST be somewhere in the default mount file system, for
...
way. This will install to `c:\workspace\fakecoin`, for example:

    make install DESTDIR=/mnt/c/workspace/fakecoin
...
causes two of the fakecoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the fakecoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the fakecoin source code).
