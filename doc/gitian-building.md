...
*Setup instructions for a Gitian build of FakeCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FakeCoin
...
- [Building FakeCoin Core](#building-fakecoin-core)
...
Clone the git repositories for fakecoin and Gitian.
...
in the fakecoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building FakeCoin Core
----------------

To build FakeCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the fakecoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fakecoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Fake-Coin/FakeCoin-Qt
...
For example:
```bash
URL=https://github.com/thrasher-/fakecoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit fakecoin=${COMMIT} --url fakecoin=${URL} ../fakecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fakecoin=${COMMIT} --url fakecoin=${URL} ../fakecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fakecoin=${COMMIT} --url fakecoin=${URL} ../fakecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the fakecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fakecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/fakecoin
SIGPATH=/some/root/path/fakecoin-detached-sigs

./bin/gbuild --url fakecoin=${BTCPATH},signature=${SIGPATH} ../fakecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fakecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fakecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fakecoin-osx-build.assert
```
...
[fakecoin-project/gitian.sigs.ltc](https://github.com/fakecoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
